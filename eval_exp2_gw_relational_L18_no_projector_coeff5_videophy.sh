...
###############################################################################
# VideoREPA Exp2 - GW Relational L18 No Projector (coeff5) - VideoPhy v1 Evaluation Pipeline
#
# This script evaluates the exp2 gw_relational_L18_no_projector trained model (checkpoint-4000)
# with proj_coeff=5 on the VideoPhy v1 benchmark.
#
# Three stages:
...
#
# Usage:
#   bash eval_exp2_gw_relational_L18_no_projector_coeff5_videophy.sh [--stage 1|2|3|all]
#
# Prerequisites:
...
EXP_DIR="${BASE_PATH}/finetune/output_dir_cogvideox-t2v-gw-align_gw_relational_exp2_gw_relational_L18_no_projector"
CHECKPOINT_STEP="checkpoint-4000"

# Inference model directory (will be created - different from previous run)
INFER_MODEL_DIR="${BASE_PATH}/infer_model/cogvideox-2b-exp2-gw-relational-L18-no-projector-coeff5"

# Output directory for generated videos (VideoPhy v1 - different from previous run)
OUTPUT_VIDEO_DIR="${INFERENCE_DIR}/output_dir/exp2_gw_relational_L18_no_projector_coeff5_videophy"
...
echo "============================================"
echo "VideoREPA Exp2 GW Relational L18 No Projector (coeff5) - VideoPhy v1 Evaluation"
echo "Stage: ${STAGE}"
...
if [ "$STAGE" = "1" ] || [ "$STAGE" = "all" ]; then
    ...
    # Force clean old results to avoid stale/corrupted files
    echo ">>> Cleaning old Stage 1 results..."
    ...
    # These weights are ONLY used during training for GW alignment loss computation.
...
if [ "$STAGE" = "3" ] || [ "$STAGE" = "all" ]; then
    ...
    CSV_NAME="exp2_gw_relational_L18_no_projector_coeff5_videophy"
...
"
    ...
    echo ""
    echo "========== VideoPhy v1 Results for Exp2 GW Relational L18 No Projector (coeff5) =========="
    echo "Checkpoint: ${EXP_DIR}/${CHECKPOINT_STEP}"
