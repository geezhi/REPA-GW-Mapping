...
###############################################################################
# VideoREPA Exp2 - VideoPhy v1 Evaluation Pipeline
#
# This script evaluates the exp2 trained model (cosine_similarity_exp2_half_data_16ep)
# on the VideoPhy v1 benchmark.
#
# Three stages:
#   Stage 1: Merge DeepSpeed checkpoint-8000 to inference-ready model
#   Stage 2: Generate VideoPhy v1 test videos (344 prompts)
#   Stage 3: Evaluate generated videos using VideoPhy v1 AutoEval
#
# Usage:
#   bash eval_exp2_videophy.sh [--stage 1|2|3|all]
#
# Prerequisites:
...
# Experiment directory (exp2: half data, 16 epochs, checkpoint-8000)
# Note: checkpoint is stored on /group/40059/zixianhuang/ (not workspace symlink)
EXP_DIR="/group/40059/zixianhuang/VideoREPA_exp2/finetune/output_dir_cogvideox-t2v-align_cosine_similarity_exp2_half_data_16ep_REPA"
CHECKPOINT_STEP="checkpoint-8000"

# Inference model directory (will be created)
# Note: avoid using ${BASE_PATH}/ckpt/ as it may be a symlink
INFER_MODEL_DIR="${BASE_PATH}/infer_model/cogvideox-2b-exp2-cosine-sim"

# Output directory for generated videos (VideoPhy v1)
OUTPUT_VIDEO_DIR="${INFERENCE_DIR}/output_dir/exp2_cosine_similarity_videophy"
...
echo "============================================"
echo "VideoREPA Exp2 - VideoPhy v1 Evaluation"
echo "Stage: ${STAGE}"
...
if [ "$STAGE" = "1" ] || [ "$STAGE" = "all" ]; then
    ...
    if [ ! -d "${INFER_MODEL_DIR}" ]; then
        ...
        sudo mkdir -p "${INFER_MODEL_DIR}"
        sudo chmod -R 777 "${INFER_MODEL_DIR}"
        ...
        for item in "${ORIG_MODEL}"/*; do
            ...
            if [ "$basename_item" != "transformer" ]; then
                sudo cp -r "$item" "${INFER_MODEL_DIR}/${basename_item}"
            fi
        done

        # Copy the merged transformer
        sudo cp -r "${EXP_DIR}/transformer" "${INFER_MODEL_DIR}/transformer"
        sudo chmod -R 777 "${INFER_MODEL_DIR}"
...
if [ "$STAGE" = "2" ] || [ "$STAGE" = "all" ]; then
    ...
    # This way if one GPU drops, it restarts immediately without waiting for others
...
if not os.path.exists(prefix):
    # Try without the videophy subdirectory
    prefix = '${OUTPUT_VIDEO_DIR}/'
...
with open('${CSV_FILE}', 'r') as f:
    ...
    for row in reader:
        if os.path.exists(row[0]):
            count += 1
print(f'Found {count} existing video files')
"
    ...
    echo ""
    echo "========== VideoPhy v1 Results for Exp2 =========="
