...
###############################################################################
# VideoREPA Exp2 - Dual Layer (L18 REPA + L5 Temporal Diff Only) - VideoPhy v1 Evaluation
#
# This script evaluates the exp2 dual_layer trained model (checkpoint-4000)
...
#
# Usage:
#   bash eval_exp2_dual_layer_videophy.sh [--stage 1|2|3|all]
#
# Prerequisites:
...
# Experiment directory (exp2: dual layer alignment, checkpoint-4000)
EXP_DIR="${BASE_PATH}/finetune/output_dir_cogvideox-t2v-align_cosine_similarity_exp2_dual_layer_L18_repa_L5_temporal_diff_only"
CHECKPOINT_STEP="checkpoint-4000"

# Inference model directory (will be created)
INFER_MODEL_DIR="${BASE_PATH}/infer_model/cogvideox-2b-exp2-dual-layer"

# Output directory for generated videos (VideoPhy v1)
OUTPUT_VIDEO_DIR="${INFERENCE_DIR}/output_dir/exp2_dual_layer_videophy"
...
echo "============================================"
echo "VideoREPA Exp2 Dual Layer (L18+L5) - VideoPhy v1 Evaluation"
echo "Stage: ${STAGE}"
...
if [ "$STAGE" = "1" ] || [ "$STAGE" = "all" ]; then
    ...
    if [ ! -d "${EXP_DIR}/transformer" ]; then
        ...
        if [ -f "${ORIG_MODEL}/transformer/config.json" ]; then
            ...
            # Update config.json to include dual-layer alignment parameters
            python -c "
import json
config_path = '../transformer/config.json'
with open(config_path, 'r') as f:
    config = json.load(f)
config['align_layer'] = 18
config['align_layer_secondary'] = 5
config['align_dims'] = [768]
config['projector_dim'] = 2048
config['_class_name'] = 'CogVideoXTransformer3DModelAlign'
with open(config_path, 'w') as f:
    json.dump(config, f, indent=2)
print('>>> Updated config.json with dual-layer alignment parameters.')
"
        fi
...
if [ "$STAGE" = "3" ] || [ "$STAGE" = "all" ]; then
    ...
    CSV_NAME="exp2_dual_layer_videophy"
...
"
    ...
    echo ""
    echo "========== VideoPhy v1 Results for Exp2 Dual Layer (L18+L5) =========="
    echo "Checkpoint: ${EXP_DIR}/${CHECKPOINT_STEP}"
