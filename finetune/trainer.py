...
class Trainer:
    ...
    def __init__(self, args: Args) -> None:
        ...
        self.state = State(
            ...
            train_frames=self.args.train_resolution[0], # max_num_frames
    ...
    def prepare_dataset(self) -> None:
        ...
        if self.args.precomputing:   # should be disable when already precomputing to avoid code execution stuck here
            logger.info("Precomputing latent for video and prompt embedding ...")
            tmp_data_loader = torch.utils.data.DataLoader(
                self.dataset,
                collate_fn=self.collate_fn,
                batch_size=1,
                num_workers=0,      # NONE multi process is used so the frames = frames.to(self.device) is ok 
            ...
            logger.info("Precomputing only mode: exiting after caching.")
            import sys
            sys.exit(0)
        else:
            pass
    ...
    def prepare_trainable_parameters(self):
        ...
        # Set trainable parameters for LoRA setting in VideoREPA
        try:
            for module in self.components.transformer.projectors:
                module.requires_grad_(True)   
            self.components.transformer.downsampler_cogvideo_output.requires_grad_(True)
        except:
            pass

        # Make the learnable TokenMerger (merger-align mode) trainable in LoRA mode.
        # In SFT mode the whole transformer is already trainable, so this is a no-op there.
        try:
            self.components.transformer.token_merger.requires_grad_(True)
        except:
            pass
    ...
    def prepare_optimizer(self) -> None:
        ...
        #     (name, param) for name, param in self.components.transformer.named_parameters() 
        ...
        # The following code in the original code (CogVideoX) should be modified to our version to avoid bugs.
        # total_training_steps = self.args.train_steps * self.accelerator.num_processes
        # num_warmup_steps = self.args.lr_warmup_steps * self.accelerator.num_processes
        
        total_training_steps = self.args.train_steps * self.args.gradient_accumulation_steps
    ...
    def prepare_trackers(self) -> None:
        ...
        # wandb disabled - no tracker initialization
        pass
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            ...
            for step, batch in enumerate(self.data_loader):
                    ...
                    if isinstance(loss, list):
                        if len(loss) == 3:
                            ...
                            if loss_REPA is not None:
                                loss = loss_diffusion + self.args.proj_coeff * (loss_REPA + loss_TRD)
                            else:
                                loss = loss_diffusion + self.args.proj_coeff * loss_TRD
                        else:
                            # REPA loss
                            loss_diffusion = loss[0]
                            loss_REPA = loss[1]
                            loss_TRD = None
                            loss = loss_diffusion + self.args.proj_coeff * loss_REPA 
                    else:
                        loss_REPA = None 
                        loss_diffusion = None
                        loss_TRD = None
                ...
                if loss_diffusion is not None:
                    if loss_REPA is not None:
                        if self.args.loss == 'cosine_margin_temporal_margin' or self.args.loss == 'cosine_margin':
                            logs["loss_REPA"] = loss_REPA.detach().item() - 0.8   # For margin
                        else:
                            logs["loss_REPA"] = loss_REPA.detach().item()
                    logs["loss"] = loss_diffusion.detach().item()
                    
                else:
                    logs["loss"] = loss.detach().item()
                    
                if loss_TRD is not None:
                    logs["loss_TRD"] = loss_TRD.detach().item()
    ...
    def validate(self, step: int) -> None:
        ...
        if accelerator.is_main_process:
            # wandb disabled - skip tracker logging
            pass
    ...
    def fit(self):
        ...
        self.initialize_vision_encoder()
        self.train()
    ...
    def initialize_vision_encoder(self):
        ...
        pass    # for models that doesn't align to vision encoder, skip this
