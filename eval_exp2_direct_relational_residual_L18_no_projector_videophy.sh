...
if [ -z "$CONDA_PREFIX" ] || [[ "$CONDA_PREFIX" != *"videorepa"* ]]; then
    echo "Activating videorepa conda environment..."
    eval "$(conda shell.bash hook 2>/dev/null)" && conda activate videorepa 2>/dev/null || true
fi

###############################################################################
# VideoREPA Exp2 - Direct Relational Residual L18 No Projector - VideoPhy v1 Evaluation Pipeline
#
# This script evaluates the exp2 direct_relational_residual_L18_no_projector trained model
# (checkpoint-4000) on the VideoPhy v1 benchmark.
#
# Key idea: Uses block residual (x_out - x_in) at layer 18 to compute direct
# relational alignment (similarity matrix comparison) without a projector.
#
# Three stages:
#   Stage 1: Merge DeepSpeed checkpoint-4000 to inference-ready model
#            (automatically removes downsampler_cogvideo_output weights)
#   Stage 2: Generate VideoPhy v1 test videos (344 prompts)
#   Stage 3: Evaluate generated videos using VideoPhy v1 AutoEval
#
# Usage:
#   bash eval_exp2_direct_relational_residual_L18_no_projector_videophy.sh [--stage 1|2|3|all]
#
# Prerequisites:
...
# Experiment directory (exp2: Direct relational residual L18 no projector, checkpoint-4000)
EXP_DIR="${BASE_PATH}/finetune/output_dir_cogvideox-t2v-direct-align_gw_relational_exp2_direct_relational_residual_L18_no_projector"
CHECKPOINT_STEP="checkpoint-4000"

# Inference model directory (will be created)
INFER_MODEL_DIR="${BASE_PATH}/infer_model/cogvideox-2b-exp2-direct-relational-residual-L18-no-projector"

# Output directory for generated videos (VideoPhy v1)
OUTPUT_VIDEO_DIR="${INFERENCE_DIR}/output_dir/exp2_direct_relational_residual_L18_no_projector_videophy"
...
echo "============================================"
echo "VideoREPA Exp2 Direct Relational Residual L18 No Projector - VideoPhy v1 Evaluation"
echo "Stage: ${STAGE}"
...
if [ "$STAGE" = "1" ] || [ "$STAGE" = "all" ]; then
    ...
    if [ -d "${INFER_MODEL_DIR}/transformer" ] && \
       [ -n "$(find "${INFER_MODEL_DIR}/transformer" -name '*.safetensors' 2>/dev/null | head -1)" ]; then
        ...
        echo ">>> Skipping Stage 1. (Delete ${INFER_MODEL_DIR} to force re-merge)"
    else
    ...
    # Step 2: Remove downsampler_cogvideo_output weights from safetensors
...
if os.path.exists(index_file):
    ...
    if keys_to_remove:
        print(f'  Found {len(keys_to_remove)} training-only keys to remove: {keys_to_remove}')
...
"
    ...
    python3 -c "
import json
with open('${INFER_MODEL_DIR}/transformer/diffusion_pytorch_model.safetensors.index.json', 'r') as f:
    index = json.load(f)
...
if [ "$STAGE" = "3" ] || [ "$STAGE" = "all" ]; then
    ...
    CSV_NAME="exp2_direct_relational_residual_L18_no_projector_videophy"
...
"
    ...
    echo ""
    echo "========== VideoPhy v1 Results for Exp2 Direct Relational Residual L18 No Projector =========="
    echo "Checkpoint: ${EXP_DIR}/${CHECKPOINT_STEP}"
