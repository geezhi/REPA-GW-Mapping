...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu_id", type=int, default=None, help="GPU device ID to use (0-7). If set, overrides CUDA_VISIBLE_DEVICES.")
    parser.add_argument("--align_layer_secondary", type=int, default=None, help="Secondary alignment layer for dual-layer models (e.g., 5)")
    parser.add_argument("--gw_model", action="store_true", default=False, help="Use base CogVideoXPipeline for GW models (no projector/downsampler needed at inference)")
    parser.add_argument("--merger_align", action="store_true", default=False, help="Use CogVideoXTransformer3DModelAlign from merger_align (with token_merger)")

    args = parser.parse_args()

    # Explicitly set GPU device if --gpu_id is provided
    if args.gpu_id is not None:
        torch.cuda.set_device(args.gpu_id)
        print(f"Using GPU {args.gpu_id} (torch.cuda.set_device({args.gpu_id}))")
    ...
    if "videophy2.csv" in args.input_file:
        ...
        # VideoPhy
        args.output_dir = args.output_dir + '/videophy'
        os.makedirs(args.output_dir, exist_ok=True)

        with open(args.input_file, "r", encoding="utf-8") as f:
            captions = [line.strip() for line in f if line.strip()]
        # Look for videophy_detailed.txt in the same directory as the input file
        detailed_path = os.path.join(os.path.dirname(os.path.abspath(args.input_file)), "videophy_detailed.txt")
    ...
    if args.generate_type == "i2v":
        ...
        if args.gw_model:
            # For GW models: no projector/downsampler needed at inference, use base pipeline
            # The downsampler_cogvideo_output is only used during training for loss computation
            pipe = CogVideoXPipeline.from_pretrained(args.model_path, torch_dtype=dtype)
        elif args.merger_align:
            # For merger_align models (with token_merger)
            from models.cogvideox_merger_align import CogVideoXTransformer3DModelAlign as MergerAlignTransformer
            from models.cogvideox_merger_align import CogVideoXPipelineAlign as MergerAlignPipeline
            
            transformer = MergerAlignTransformer.from_pretrained(
                args.model_path, subfolder="transformer",
                align_layer=18, align_dims=[768],
            ).to(dtype)
            
            pipe = MergerAlignPipeline.from_pretrained(
                args.model_path, torch_dtype=dtype, transformer=transformer
            )
        else:
            # For VideoREPA inference (with projector)
            from models.cogvideox_align import CogVideoXPipelineAlign, CogVideoXTransformer3DModelAlign
            
            # First, load the custom transformer directly (avoids config.json _class_name mismatch)
            transformer = CogVideoXTransformer3DModelAlign.from_pretrained(
                args.model_path, subfolder="transformer",
                align_layer=18, align_layer_secondary=args.align_layer_secondary, align_dims=[768], projector_dim=2048
            ).to(dtype)
            
            # Then load the pipeline with the custom transformer injected
            pipe = CogVideoXPipelineAlign.from_pretrained(
                args.model_path, torch_dtype=dtype, transformer=transformer
            )
    else:
    ...
    # Move pipeline to the specified GPU
    ...
    if "videophy2.csv" in args.input_file:
        ...
        for idx, (caption, long_caption) in tqdm(enumerate(zip(captions, long_captions)), total=len(captions), desc="Generating videos"):
            ...
            # Use try-except to handle race condition when multiple GPUs try to create the same lock
            try:
                os.makedirs(output_path.replace('.mp4', '') + '.lock')
            except FileExistsError:
                print(f'{output_path} lock grabbed by another process, skipping.')
                continue
            ...
            try:
                ...
                logging.error(f"Failed to generate video {idx}: {output_path}. Error: {e}")
                # Clean up incomplete output file if exists
                if os.path.exists(output_path):
                    os.remove(output_path)
                # Clear CUDA cache to recover from OOM
        ...
        fail_count = 0
        max_consecutive_fails = 3  # Exit if 3 consecutive failures (likely GPU dead)
        for idx, (caption, long_caption) in tqdm(enumerate(zip(captions, long_captions)), total=len(captions), desc="Generating videos"):
            output_path = os.path.join(args.output_dir, f"{'_'.join(caption.rstrip('.').split(' '))}.mp4")
            if os.path.exists(output_path):
            ...
            try:
                ...
                fail_count += 1
                logging.error(f"Failed to generate video {idx}: {output_path}. Error: {e} (consecutive fails: {fail_count})")
                # Clean up incomplete output file if exists
                if os.path.exists(output_path):
                    os.remove(output_path)
                # Clear CUDA cache to recover from OOM
                ...
                if fail_count >= max_consecutive_fails:
                    logging.error(f"Too many consecutive failures ({max_consecutive_fails}), GPU likely dead. Exiting.")
                    break
            finally:
                if os.path.exists(output_path.replace('.mp4', '') + '.lock'):
                    os.rmdir(output_path.replace('.mp4', '') + '.lock')
