...
###############################################################################
# VideoREPA Exp2 - Temporal Diff Only - VideoPhy v1 Evaluation Pipeline
#
# This script evaluates the exp2 temporal_diff_only trained model (checkpoint-4000)
...
#
# Usage:
#   bash eval_exp2_temporal_diff_only_videophy.sh [--stage 1|2|3|all]
#
# Prerequisites:
...
# Experiment directory (exp2: temporal_diff_only alignment, checkpoint-4000)
EXP_DIR="${BASE_PATH}/finetune/output_dir_cogvideox-t2v-align_cosine_similarity_temporal_diff_only_exp2_temporal_diff_only_alignment"
CHECKPOINT_STEP="checkpoint-4000"

# Inference model directory (will be created)
INFER_MODEL_DIR="${BASE_PATH}/infer_model/cogvideox-2b-exp2-temporal-diff-only"

# Output directory for generated videos (VideoPhy v1)
OUTPUT_VIDEO_DIR="${INFERENCE_DIR}/output_dir/exp2_temporal_diff_only_videophy"
...
echo "============================================"
echo "VideoREPA Exp2 Temporal Diff Only - VideoPhy v1 Evaluation"
echo "Stage: ${STAGE}"
...
if [ "$STAGE" = "3" ] || [ "$STAGE" = "all" ]; then
    ...
    CSV_NAME="exp2_temporal_diff_only_videophy"
...
"
    ...
    echo ""
    echo "========== VideoPhy v1 Results for Exp2 Temporal Diff Only =========="
    echo "Checkpoint: ${EXP_DIR}/${CHECKPOINT_STEP}"
