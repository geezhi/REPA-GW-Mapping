...
# GW-Gram Alignment Configuration
# Key idea: Use GW transport plan T:(D1,D2) to project teacher features into
# student's dimension space, then compute Gram matrix MSE in that shared space.
# No learnable projector; the projection comes from GW optimization.
GW_GRAM_ARGS=(
    --loss gw_gram
    ...
    --proj_coeff 0.5
    --margin 0.0
    # GW parameters for dimension-level transport
    --gw_reg 0.1
    --gw_outer_iters 50
    --gw_sinkhorn_iters 100
    --gw_sample_size 0
    --gw_update_interval 1
    --gw_distance_type cosine
    --gw_outer_tol 1e-4
