...
# VideoREPA Configuration - Temporal Difference Alignment
# Key idea: Instead of aligning absolute frame features, align inter-frame differences
# (motion direction). First frame aligns original features, subsequent frames align
# frame[t] - frame[t-1], capturing motion dynamics rather than static appearance.
VideoREPA_ARGS=(
    --loss cosine_similarity_temporal_diff
    --align_models VideoMAEv2
    --align_layer 18
    --align_dims 768
    --proj_coeff 0.5
    --margin 0.1
    --comment 'exp2_temporal_diff_alignment'
    --learning_rate 2e-6
)
