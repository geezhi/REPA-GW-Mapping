...
class BaseT2VDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if '-align' in self.trainer.args.model_name:
            # Strip everything after base model name (e.g., "cogvideox-t2v-gw-align" -> "cogvideox-t2v")
            model_name_tmp = self.trainer.args.model_name[:self.trainer.args.model_name.index('-align')]
            # Remove any variant suffix after "t2v" or "i2v" (e.g., "cogvideox-t2v-gw" -> "cogvideox-t2v")
            for base_suffix in ['-t2v', '-i2v']:
                if base_suffix in model_name_tmp:
                    model_name_tmp = model_name_tmp[:model_name_tmp.index(base_suffix) + len(base_suffix)]
                    break
        ...
        # Skip uncached samples when using multiprocessing (num_workers > 0)
        # to avoid CUDA re-initialization errors in forked subprocesses
        if not prompt_embedding_path.exists() or not (encoded_video_path.exists() and encoded_frame_idx_path.exists()):
            import random
            alt_index = random.randint(0, len(self.prompts) - 1)
            if alt_index == index:
                alt_index = (index + 1) % len(self.prompts)
            logger.debug(f"Skipping uncached sample {index}, using {alt_index} instead", main_process_only=False)
