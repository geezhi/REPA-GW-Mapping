...
# Fixed Random Projection + Temporal Difference Alignment
#
# Strategy:
# 1. Token downsampling: avg_pool2d (30x45 -> 10x15), zero parameters
# 2. Dimension projection: frozen random W (1920 -> 768), zero learnable parameters
# 3. Temporal difference: student_diff[t] = proj(student[t]) - proj(student[t-1])
#                         teacher_diff[t] = teacher[t] - teacher[t-1]
# 4. Per-token cosine loss on frame DIFFERENCES: mean(1 - cos(student_diff, teacher_diff))
#
# Key insight: Combines the benefits of:
# - Fixed projection (JL lemma, 100% gradient to backbone)
# - Temporal difference (aligns motion/dynamics, not static appearance)
#
# This provides per-token MOTION supervision through a frozen random channel.
ALIGN_ARGS=(
    --loss fixed_proj_temporal_diff
    ...
    --comment 'exp6_fixed_random_proj_temporal_diff'
    --learning_rate 2e-6
)
