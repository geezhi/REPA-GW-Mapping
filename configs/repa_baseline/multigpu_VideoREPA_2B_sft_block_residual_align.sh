...
# VideoREPA Configuration - Block Residual Alignment
# Key idea: Instead of aligning the full hidden_states (x_out) at layer 18,
# we align the block residual (x_out - x_in), which represents the incremental
# contribution of that specific transformer block. This focuses the alignment
# on what the layer *learns* rather than the accumulated representation.
# Uses standard REPA cosine similarity loss on the block residual.
VideoREPA_ARGS=(
    ...
    --margin 0.1
    --align_residual
    --comment 'exp2_block_residual_L18_repa'
    --learning_rate 2e-6
)
