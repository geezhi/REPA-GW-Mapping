...
# VideoREPA Configuration - Attention Residual Alignment
# Key idea: Instead of aligning the full hidden_states (x_out) or the full block residual,
# we align only the attention residual (gate_msa * attn_output). This isolates the
# contribution of the self-attention mechanism, focusing alignment on the attention
# pattern's learned representation without interference from the FFN branch.
# This is more fine-grained than block residual alignment.
# Uses standard REPA cosine similarity loss on the attention residual.
VideoREPA_ARGS=(
    ...
    --margin 0.1
    --align_attn_residual
    --comment 'exp2_attn_residual_L18_repa'
    --learning_rate 2e-6
)
