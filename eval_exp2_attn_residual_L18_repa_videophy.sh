...
###############################################################################
# VideoREPA Exp2 - Attn Residual L18 REPA - VideoPhy v1 Evaluation Pipeline
#
# This script evaluates the exp2 attn_residual_L18_repa trained model (checkpoint-4000)
...
#
# Usage:
#   bash eval_exp2_attn_residual_L18_repa_videophy.sh [--stage 1|2|3|all]
#
# Prerequisites:
...
# Experiment directory (exp2: attn residual L18 REPA, checkpoint-4000)
EXP_DIR="${BASE_PATH}/finetune/output_dir_cogvideox-t2v-align_cosine_similarity_exp2_attn_residual_L18_repa"
CHECKPOINT_STEP="checkpoint-4000"

# Inference model directory (will be created)
INFER_MODEL_DIR="${BASE_PATH}/infer_model/cogvideox-2b-exp2-attn-residual-L18-repa"

# Output directory for generated videos (VideoPhy v1)
OUTPUT_VIDEO_DIR="${INFERENCE_DIR}/output_dir/exp2_attn_residual_L18_repa_videophy"
...
echo "============================================"
echo "VideoREPA Exp2 Attn Residual L18 REPA - VideoPhy v1 Evaluation"
echo "Stage: ${STAGE}"
...
if [ "$STAGE" = "3" ] || [ "$STAGE" = "all" ]; then
    ...
    CSV_NAME="exp2_attn_residual_L18_repa_videophy"
...
"
    ...
    echo ""
    echo "========== VideoPhy v1 Results for Exp2 Attn Residual L18 REPA =========="
    echo "Checkpoint: ${EXP_DIR}/${CHECKPOINT_STEP}"
